...
TEhelp <- function(topic = 'package') {
  ...
  ## error functions
  {
    ...
    if (topic == 'errFun=bernoulli') {
      cat('help page pending. ')
    }
    if (topic == 'errFun=rmse') {
      cat('help page pending. ')
    }
    helpTable <-
      rbind(helpTable,
            data.frame(
              topic = 'errFun=exGauss_tau',
              description = paste0(
  ...
  if (any(helpTable$topic == topic)) {
    cat(levels(droplevels(helpTable[helpTable$topic == topic, 'description'])))
  } else{
    cat(
      "The topic does not have an entry in TEhelp. Please refer to ?TEhelp and/or TEhelp('topics')."
    )
  }
