...
As a rough simulation of one participants' data, we will generate by-trial accuracies that linearly increase over the first 40 trials and then are stable in the last 40 trials
...
`Fit Values` prints the fit parameter values.
...
`Test of change in nonindependence` prints the rank correlation between the null model residuals and the time variable, the rank correlation between the full model residuals and the time variable, the ratio between the absolute values of these correlations, and the p value as calculated with `psych::r.test()`.
